...
#define ARRAY_MAX 8 // arrayLen in go
#define BUF 8192
...
/* helper to generate eventCodes:
wget  -q  https://raw.githubusercontent.com/wayland-project/libinput/master/include/linux/linux/input-event-codes.h  -O - |  grep "#define KEY_"  |while read a b c d; do
	if egrep -iq "[a-z]" <<< $c; then
		if  egrep -iq "0x" <<< $c ; then
			:
		else
			continue
		fi
...
const (
	...
	arrayLen    = 8 // max key+key+key events: 8
	sampleConf  = `
Sample Config: 
# swipe config file.
